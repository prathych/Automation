...
public class LongestWord {
	...
	public Words findLongestWord(String findLongestString) {
		//Trimming the string if it has space values and handling the only space strings
		...
		if(!input.equals(null) && !input.isEmpty()) {
			...
			for(int i=0; i<splitStr.length; i++) {
				...
				if(tempLength >= words.length) {
					/*Checking if the new string in the loop has a greater length than before length.
					 * If the condition is satisfied we are clearing the array List 
					 * to add new highly added values.
					 */
